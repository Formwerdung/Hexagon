  ...
  */
  public static function getSingleton() {
    if (! isset(self::$instance)) {
      self::$instance = new AdminNoticeHelper();
    }
  ...
  public function printNotices() {
    foreach (array( 'update', 'error' ) as $type) {
      if (count($this->notices[ $type ])) {
        ...
        require(dirname(__DIR__) . '/views/admin_notice.php');
...
} // end AdminNoticeHelper

AdminNoticeHelper::getSingleton(); // Create the instance immediately to make sure hook callbacks are registered in time

function addNotice($message, $type = 'update') {
  AdminNoticeHelper::getSingleton()->enqueue($message, $type);
}
